...
export const PageHero: React.FC<{ title: string; subtitle?: string; badge?: string }> = ({ title, subtitle, badge }) => (
  <section className="bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-600 text-white">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
      {badge && (
        <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wider mb-4">
          {badge}
        </span>
      )}
      <h1 className="text-3xl sm:text-4xl font-bold max-w-3xl">{title}</h1>
      {subtitle && <p className="mt-3 text-blue-100 max-w-2xl text-sm sm:text-base">{subtitle}</p>}
    </div>
  </section>
);
...
export const SectionHeading: React.FC<{ title: string; subtitle?: string; center?: boolean }> = ({ title, subtitle, center }) => (
  <div className={center ? 'text-center' : ''}>
    ...
    {subtitle && <p className="mt-2 text-gray-500 max-w-2xl text-sm sm:text-base">{subtitle}</p>}
  </div>
);
...
export const EmptyState: React.FC<{ title?: string; message?: string }> = ({ title = 'Nothing here yet', message = 'Content is being prepared. Please check back soon.' }) => (
  <div className="rounded-2xl border border-dashed border-gray-200 p-12 text-center">
    ...
    <p className="text-sm text-gray-400 mt-1.5 max-w-sm mx-auto">{message}</p>
  </div>
);

export const PageNotFound: React.FC<{ title?: string }> = ({ title = 'Page not found' }) => (
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
    <p className="text-6xl font-extrabold text-blue-100">404</p>
    <h1 className="mt-2 text-2xl font-bold text-gray-900">{title}</h1>
    <p className="mt-2 text-gray-500 text-sm">The page you're looking for doesn't exist or has been removed.</p>
    <Link to={isPreviewMode() ? '/preview?page=home' : '/'} className="mt-6 inline-flex items-center px-5 py-2.5 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-colors">
      Back to home
    </Link>
  </div>
);
  ...
  return (
    <div className="space-y-6">
      {list.map((block, i) => {
        ...
        switch (block.type) {
          case 'heading':
            return <h2 key={i} className="text-2xl font-bold text-gray-900 mt-2">{block.text}</h2>;
          case 'subheading':
            return <h3 key={i} className="text-lg font-semibold text-gray-900">{block.text}</h3>;
          case 'paragraph':
          ...
          case 'image':
            return block.src ? (
              <figure key={i}>
                ...
                {block.alt && <figcaption className="text-xs text-gray-400 mt-2">{block.alt}</figcaption>}
              </figure>
            ) : null;
          case 'quote':
            return (
              <blockquote key={i} className="border-l-4 border-blue-600 pl-4 italic text-gray-600">
                {block.text}
              </blockquote>
            );
          default:
